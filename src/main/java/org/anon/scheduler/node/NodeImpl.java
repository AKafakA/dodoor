...
public class NodeImpl implements Node {
    ...
    // Removed host-load simulation; capacity is taken directly from config
    ...
    public void initialize(Configuration staticConfig, NodeThrift nodeThrift, JSONObject nodeTypeConfig,
        ...
        boolean restrictFifo = staticConfig.getBoolean(DodoorConf.RESTRICT_FIFO, DodoorConf.DEFAULT_RESTRICT_FIFO);
        int configuredSlots = nodeTypeConfig.getInt(DodoorConf.NUM_SLOTS);
        double systemCores = Resources.getSystemCoresCapacity(nodeTypeConfig);
        int systemMemoryMb = Resources.getMemoryMbCapacity(nodeTypeConfig);
        int systemDiskGb = Resources.getSystemDiskGbCapacity(nodeTypeConfig);

        int numSlots = Math.max(configuredSlots, 1);
        double loadedCores = Math.max(systemCores, 1);
        int loadedMemoryMb = Math.max(systemMemoryMb, 1);
        int loadedDiskGb = Math.max(systemDiskGb, 0);
