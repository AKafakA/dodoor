...
import org.anon.scheduler.utils.*;
...
public class TaskLauncherService {
    ...
    private class TaskLaunchRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                String command = executeLaunchTask(_task);
                process = Runtime.getRuntime().exec(command);
                boolean finishedInTime;

                // Wait for the process to complete, with a timeout
                if (_task._taskType.equals(TaskTypeID.SIMULATED.toString())) {
                    // Simulated tasks: wait with timeout
                    finishedInTime = process.waitFor(_task._durationInMs, TimeUnit.MILLISECONDS);
                } else {
                    // FunctionBench tasks: wait for actual Docker container completion
                    // FIXED: Now that we removed -d flag, this waits for actual task execution
                    // Add 2x expected duration as timeout to handle variance
                    long timeoutMs = _task._durationInMs * 2 + 10000; // 2x + 10s buffer
                    finishedInTime = process.waitFor(timeoutMs, TimeUnit.MILLISECONDS);
                }

                if (!finishedInTime) {
                    LOG.warn("Task {} timed out after {} ms.",
                    ...
                    if (exitCode != 0) {
                        ...
                        String errorOutput = errorLines.isEmpty() ? "No error output" :
                                String.join("\n", errorLines);
                        LOG.error("Task {} with command {} failed. Error output: {}",
                                new Object[]{_task._taskId, command, errorOutput});
                    } else {
                        LOG.debug("Task {} completed successfully", _task._taskId);
                    }
                }
            ...
            tryToUnlaunchTask(taskStartTime, _task);
        }

        /** Executes to launch a task */
        private String executeLaunchTask(TaskSpec task) throws IOException, InterruptedException {
            ...
            if (taskType.equals(TaskTypeID.SIMULATED.toString())) {
                ...
                // the maximal duration is 1 second
                double durationInSec = Math.max((durationInMs * 1.0) / 1000, 1.0);
    ...
    public void recordHolBlocked() {
        _nodeServiceMetrics.holBlockedOnce();
    }

    public void recordHolBlockedDuration(long blockedMs) {
        _nodeServiceMetrics.holBlockedFor(blockedMs);
    }

    private void tryToUnlaunchTask(long taskStartTime, TaskSpec task) {
        ...
        LOG.debug("Task {} processing completed in {} ms", task._taskId, taskEndTime - taskStartTime);
    }

    public int getActiveTasks() {
        return _activeTasks.get();
    }
    ...
    private static String generatePythonCommand(String pythonScriptPath, String dockerImageName,
        ...
        // FIXED: Removed -d (detached) flag so process.waitFor() waits for actual task completion
        // Previously: docker run -d returned immediately after starting container (~300ms)
        // Now: docker run waits for container to finish (actual execution time: 200ms-5000ms)
        String command = String.format("docker run --rm --cpus %d --memory %dm -v %s:/app %s python3 %s %s",
                (int)cpuCores, memory, hostDir, dockerImageName, pythonScriptPath, taskMode);
        LOG.debug("Generated python command: {}", command);
        return command;
    }
}
