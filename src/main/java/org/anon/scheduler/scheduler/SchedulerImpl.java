...
import org.anon.scheduler.scheduler.taskplacer.TaskPlacer;
...
public class SchedulerImpl implements Scheduler{
    ...
    public void submitJob(TSchedulingRequest request) throws TException {
        ...
        for (int i = 0; i < _roundOfReservations; i++) {
            ...
            Map<InetSocketAddress, List<TEnqueueTaskReservationRequest>> placedTasks = handleJobSubmission(request, start, i);
        ...
        // REPLAY: Log preserved nodes per task (DEBUG)
        if (_nodePreservedForTask != null) {
            long ts = System.currentTimeMillis();
            for (TTaskSpec task : request.tasks) {
                Set<InetSocketAddress> preserved = _nodePreservedForTask.get(task.taskId);
                if (preserved != null && !preserved.isEmpty()) {
                    String nodes = preserved.stream().map(InetSocketAddress::getHostName).collect(Collectors.joining("|"));
                    LOG.debug("REPLAY reservation_set t={} task={} nodes={} sched={}",
                            new Object[]{ts, task.taskId, nodes, _address.host});
                }
            }
        }
    ...
    private void updateDataStoreLoad(int numTasksBefore, TSchedulingRequest request,
        ...
        // Capture current counter value atomically to prevent race conditions
        int currentTaskCount = _counter.get();
        boolean needToUpdateDataStore = numTasksBefore / _numTasksToUpdateDataStore != currentTaskCount / _numTasksToUpdateDataStore;
        for (InetSocketAddress nodeEnqueueAddress : mapOfNodesToPlacedTasks.keySet()) {
            ...
            if (needToUpdateDataStore) {
                LOG.debug("{} tasks scheduled. and need to update the datastore from scheduler side", currentTaskCount);
    ...
    public Map<InetSocketAddress, List<TEnqueueTaskReservationRequest>> handleJobSubmission(TSchedulingRequest request,
        ...
        LOG.info("Handling job submission for request: {} round: {}", request.requestId, round);
        ...
        Map<TEnqueueTaskReservationRequest, InetSocketAddress> enqueueTaskReservationRequests
                = _taskPlacer.getEnqueueTaskReservationRequests(request, _loadMapEqueueSocketToNodeState, _address, round);
    ...
    public void taskFinished(TFullTaskId taskId, long nodeWallTime) throws TException {
        ...
        _schedulerServiceMetrics.taskFinished(taskId.taskId, taskDuration, nodeWallTime, taskId.durationInMs);
        if (SchedulerUtils.isLateBindingScheduler(_schedulingStrategy)) {
            long totalScheduledTime = _taskEnqueueTime.getOrDefault(taskId.taskId, 0L) +
                    _taskConfirmedTime.getOrDefault(taskId.taskId, 0L);
            _schedulerServiceMetrics.taskScheduled(totalScheduledTime, taskId.taskId);
        }
    }
    ...
    public synchronized boolean confirmTaskReadyToExecute(TFullTaskId taskId, String nodeAddressStr) throws TException {
        ...
        if (_schedulingStrategy.equals(DodoorConf.SPARROW_SCHEDULER)) {
            if (!_nodePreservedForTask.containsKey(taskId.taskId)) {
                ...
                LOG.debug("REPLAY confirm_decision t={} task={} node={} accepted={} ",
                        new Object[]{System.currentTimeMillis(), taskId.taskId, nodeAddressStr, false});
                return false;
            }
                ...
                try {
                    ...
                    for (InetSocketAddress address : preservedNodes) {
                        ...
                        try {
                            ...
                            LOG.debug("REPLAY reservation_cancel t={} task={} node={}",
                                    new Object[]{System.currentTimeMillis(), taskId.taskId, address.getHostName()});
                        } catch (Exception e) {
                            throw new RuntimeException(e);
                        }
                    }
                    LOG.debug("REPLAY confirm_decision t={} task={} node={} accepted={}",
                            new Object[]{System.currentTimeMillis(), taskId.taskId, nodeAddressStr, true});
                    return true;
                } catch (Exception e) {
                    throw new RuntimeException(e);
                }
    ...
    private class EnqueueTaskReservationCallback implements AsyncMethodCallback<Boolean> {
        ...
        public void onComplete(Boolean aBoolean) {
            ...
            if (!SchedulerUtils.isLateBindingScheduler(_schedulingStrategy)) {
                _schedulerServiceMetrics.taskScheduled(taskEnqueueTime, _taskId);
            } else {
    ...
    private class GetNodeStateWithUpdateCallBack implements AsyncMethodCallback<org.anon.scheduler.thrift.TNodeState> {
