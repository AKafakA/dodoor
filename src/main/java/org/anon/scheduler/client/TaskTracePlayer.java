package org.anon.scheduler.client;
...
public class TaskTracePlayer {
    ...
    private static class TaskLaunchRunnable implements Runnable {
        ...
        public void run(){
            ...
            } else if (start > _startTime && _addTimelineDelay) {
                LOG.warn("Task " + _taskId + " is scheduled to start at " + _startTime +
                        " but the current time is " + start + ". No delay will be added.");
            }
            try {
    ...
    public static void main(String[] args) throws ConfigurationException, TException, IOException, MathException {
        ...
        parser.accepts("n", "the number of tasks to replay, 0 means all tasks in the trace").
        ...
        int numTasksToReplay = allLines.size();
        if (options.has("n")) {
            numTasksToReplay = (int) options.valueOf("n");
            if (numTasksToReplay <= 0 || numTasksToReplay > allLines.size()) {
                numTasksToReplay = allLines.size();
            }
        }
        ...
        for (String line : allLines) {
            if (i >= numTasksToReplay) {
                LOG.info("Reached the limit of tasks to replay: " + numTasksToReplay);
                break;
            }
            i++;
