...
for scheduler_name in os.listdir(composited_node_host_dir):
    ...
    # if not (scheduler_type in baseline_scheduler_type or scheduler_type in compared_scheduler_type):
    #     continue
...
for scheduler_name in os.listdir(scheduler_host_dir):
    for scheduler_log_file in os.listdir(os.path.join(scheduler_host_dir, scheduler_name)):
        if "scheduler_metrics" in scheduler_log_file:
            ...
            submitted_tasks = nodes_metrics.get_submitted_tasks()
            ...
            print("scheduler: {},"
                  ...
                  " scheduling latency p999: {}, "
                  " final number messages :{}, ".format(scheduler_type,
                                                        task_e2e_makespan_duration_avg[max_checkpoints - 1] / 1000,
                                                        task_e2e_makespan_duration_p50[max_checkpoints - 1] / 1000,
                                                        task_e2e_makespan_duration_p99[max_checkpoints - 1] / 1000,
                                                        task_e2e_makespan_duration_p999[max_checkpoints - 1] / 1000,
                                                        e2e_latency_avg[max_checkpoints - 1],
                                                        e2e_latency_p50[max_checkpoints - 1],
                                                        e2e_latency_p99[max_checkpoints - 1],
...
average_waiting_time_fig, (average_waiting_time_ax1, average_waiting_time_ax2) = plt.subplots(2, 1, sharex=True)
average_waiting_time_fig.subplots_adjust(hspace=0.05)
average_waiting_time_fig.supxlabel('10 seconds')
average_waiting_time_ax1.set_ylabel("Diff from Random")
...
average_waiting_time_fig.savefig("{}/average_waiting_time.png".format(target_dir))