...
def generate_azure_data(azure_data_path,
    ...
    azure_data_generator = AzureDataGenerator(azure_data_path, machine_ids=range(1, 36),
                                              ...
                                              distribution_type=distribution_type,
                                              burstiness=burstiness)
    data = azure_data_generator.generate(num_records, 0, max_duration,[0, 14],
                                         max_cores=max_cores,
                                         max_memory=max_memory, max_disk=0,
                                         noise_intensity=noise_intensity)
    azure_data_generator.write_data_target_output(data, azure_output_path, noise_intensity=noise_intensity)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_records", type=int, default=220000,
                        help="Number of records to generate")
    parser.add_argument("--generated_dataset", nargs='+',
                        default=["function_bench"], )
    ############ Options for Azure data generation ############
    ...
    parser.add_argument("--projected_host_cores", type=int, default=96,
                        help="Projected cores for the Azure trace data, used to convert to real cores")
    parser.add_argument("--projected_host_memory", type=int, default=672 * 1024,
    ...
    parser.add_argument("--max_memory", type=int, default=32 * 1024,
                        help="Maximum memory for the generated records in MB for smallest hosts for "
                             "Used by Azure as a task filter to avoid unaccommodated tasks")
    parser.add_argument("--max_duration", type=int, default=int(1000 * 60 * 10),
    ...
    parser.add_argument("--noise_intensity", type=float, default=0.0,
                        help="Noise intensity to add noise to the Azure trace data' duration")
    ############ Options for Function Bench trace generation ############
    ...
    parser.add_argument("--function_bench_trace_output_path", type=str,
                        default="deploy/resources/data/function_bench",
    ...
    parser.add_argument("--function_bench_task_mode_distribution", type=float, nargs='+',
                        ...
                        help="Distribution of task modes for Function Bench trace"
                             "to control the ratio of long/medium/short tasks")
    ############ Options for Serverless trace generation ############
    ...
    if "azure" in args.generated_dataset:
        ...
        generate_azure_data(
            ...
            projected_host_cores=args.projected_host_cores,
            projected_host_memory=args.projected_host_memory,
    ...
    if "function_bench" in args.generated_dataset:
        ...
        function_bench_task_mode_distribution = {
            "short": args.function_bench_task_mode_distribution[0],
        ...
        # rename the output path to include distribution of task modes in digit
        # for example, if the distribution is [0.6, 0.3, 0.1], the output path will be
        # 60-30-10
        output_path = output_path + f"/" + \
                      f"{int(function_bench_task_mode_distribution['short']*100)}" + \
                      f"-{int(function_bench_task_mode_distribution['medium']*100)}" + \
                      f"-{int(function_bench_task_mode_distribution['long']*100)}.csv"
