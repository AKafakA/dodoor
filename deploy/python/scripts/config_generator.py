...
def parse_args():
    ...
    parser.add_option("--start_tracking_task_makespan_id", default=0,
                      help="The task id to start tracking the makespan of the tasks in the trace. "
                           "This is used to track the makespan of the tasks in the trace from a specific task id.")
    parser.add_option("--end_tracking_task_makespan_id", default=7000,
                        help="The task id to end tracking the makespan of the tasks in the trace. "
                             "This is used to track the makespan of the tasks in the trace until a specific task id.")

    parser.add_option("--log_per_task_metrics", default=False,
                      help="Whether to log the per task metrics in the scheduler")
...
def main():
    ...
    # Removed host load config
    file.write("docker.image.name = {} \n".format(options.docker_image_name))
    file.write("dodoor.root.dir = {} \n"
               .format(f"/users/{options.user_name_test_host}/dodoor/deploy/python/function_bench"))

    file.write("start.tracking.task.makespan.id = {} \n".format(options.start_tracking_task_makespan_id))
    file.write("end.tracking.task.makespan.id = {} \n".format(options.end_tracking_task_makespan_id))

    file.write("restrict.fifo = {} \n".format(options.restrict_fifo))

    file.write("log.per.task.metrics = {} \n".format(options.log_per_task_metrics))
