...
class NodeMetrics:
    ...
    def parse(self):
        ...
        # Determine a robust series length using only metrics that were actually logged.
        # Simulation logs may omit some histograms; fall back to min over non-empty series.
        non_empty_lengths = [len(metric) for metric in self.metrics.values() if len(metric) > 0]
        self.length = min(non_empty_lengths) if non_empty_lengths else 0
