...
def plot_cdf(data_dict, metric_name, parameter_name, experiment_name, output_dir, aggregated_df=None):
    ...
    ax.set_xlabel(f'{metric_name.capitalize()} Latency (ms)', fontsize=20)
    ...
    if stats_text:
        ...
        if metric_name == 'makespan':
            ...
            ax.text(0.98, 0.35, textstr, transform=ax.transAxes, fontsize=15,
                    verticalalignment='bottom', horizontalalignment='right',
                    bbox=props, family='monospace')
...
def plot_cdf_on_axis(ax, data_dict, metric_name, parameter_name, aggregated_df=None):
    ...
    for idx, param_value in enumerate(sorted_params):
        ...
        if metric_name == 'makespan':
            ...
            stat_line_2 = f"P99.9={p999_val:.1f}, RPC={rpc_count} \n"
    ...
    if stats_text:
        ...
        if metric_name == 'makespan':
            ax.text(0.98, 0.02, textstr, transform=ax.transAxes, fontsize=14,
                    verticalalignment='bottom', horizontalalignment='right',
                    bbox=props, family='monospace')
        else:
            ax.text(0.98, 0.35, textstr, transform=ax.transAxes, fontsize=14,
                    verticalalignment='bottom', horizontalalignment='right',
                    bbox=props, family='monospace')
