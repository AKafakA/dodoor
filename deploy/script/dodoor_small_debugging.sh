...
parallel-ssh -h deploy/resources/host_addresses/caelum/small_caelum -i  "cd dodoor && git add . && git stash && git checkout exp && git pull && python3 ~/dodoor/deploy/python/scripts/config_generator.py -d ~/dodoor/deploy/resources/host_addresses/caelum/caelum_scheduler_ip -n ~/dodoor/deploy/resources/host_addresses/caelum/small_caelum_host_ip  -s ~/dodoor/deploy/resources/host_addresses/caelum/caelum_scheduler_ip --replay_with_delay True --scheduler-ports $SCHEDULER_PORTS --use-configable-address True --batch-size $BATCH_SIZE --beta $BETA --node-num-tasks-update $NODE_NUM_TASKS_UPDATE --scheduler-num-tasks-update $SCHEDULER_NUM_TASKS_UPDATE --cores $CORES --memory $MEMORY --scheduler-type $SCHEDULER_TYPE && sh rebuild.sh && rm ~/*.log && rm ~/*.out"
