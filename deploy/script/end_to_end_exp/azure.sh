...
SCHEDULERS="${SCHEDULERS:-powerOfTwo dodoor prequal random}"
LOG_LEVEL="${LOG_LEVEL:-info}"

# Dataset is constant and not iterated over.
DATA_PATH="${DATA_PATH:-deploy/resources/data/azure_data}"
...
LOG_DIR_PREFIX="${LOG_DIR_PREFIX:-azure}"
NUM_REQUESTS="${NUM_REQUESTS:-4000}"   # paper-exact
CODE_UPDATE="${CODE_UPDATE:-false}"
RUN_EXPERIMENT="${RUN_EXPERIMENT:-true}"
EXPERIMENT_TIMEOUT_IN_MIN="${EXPERIMENT_TIMEOUT_IN_MIN:-600}"
QPS="${QPS:-1 5 10 20}"
...
for restrict_fifo in $RESTRICT_FIFO; do
  ...
  for qps in $QPS; do
      for max_duration in $MAX_DURATIONS; do
        ...
        if [ "$NOISE_INTENSITY" != "0.0" ]; then
          data_path="${data_path}_noise_${NOISE_INTENSITY}"
        fi
