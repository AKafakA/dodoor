...
DATA_PATH="deploy/resources/data/serverless_small"
DURATION_WEIGHT=0.5

parallel-ssh -h deploy/resources/host_addresses/caelum/small_caelum  -i "rm ~/*.log && rm ~/*.out"

parallel-ssh -h deploy/resources/host_addresses/caelum/small_caelum -i "cd dodoor && git fetch --all && git add . && git stash && git checkout test_serverless && git pull && sh rebuild.sh"
