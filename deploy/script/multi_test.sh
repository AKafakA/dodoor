...
for i in "sparrow" "random";
  ...
  for slot_size in 4
    do
       ...
       sh deploy/script/single_test_cloudlab.sh $i $BATCH_SIZE $slot_size $BETA
       sleep ${SLEEP_TIME}
    ...
    for batch_size in 100
      ...
      for cpu_weight in 0.25 1.0 4.0
        do
           ...
           sleep ${SLEEP_TIME}
           python3 deploy/python/scripts/collect_logs.py $i $batch_size $BETA $slot_size $cpu_weight
           parallel-ssh -h deploy/resources/host_addresses/cloud_lab/test_host  -i "rm ~/*.log && rm ~/*.out"
        done
      done
    done
